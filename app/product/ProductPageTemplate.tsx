...
export type FrameId = 'JJ04B' | 'JJ20B' | 'JJ21G' | 'JJ22B' | 'JJ23Grey' | 'JJ24Grey' | 'H1Black' | 'H1Blue' | 'H1Red' | 'H1Silver' | 'H2'
...
export const newtonFrameConfigs: FrameConfig[] = [
  ...
  },
  {
    ...
    colors: [{ name: 'Silver', value: 'silver', image: '/Frames/H1Silver.png' }],
  },
  {
    id: 'H2',
    label: 'H2',
    baseImage: '/Frames/H2.png',
    colors: [{ name: 'Default', value: 'default', image: '/Frames/H2.png' }],
  },
]
...
export default function ProductPageTemplate({ config }: { config: ProductPageConfig }) {
  ...
  const frameConfigs = config.slug === 'newton' ? newtonFrameConfigs : defaultFrameConfigs
  const defaultFrameId = config.slug === 'newton' ? 'H1Black' : 'JJ23Grey'
  const defaultColor = config.slug === 'newton' ? 'black' : 'grey'
  
  const [selectedFrameId, setSelectedFrameId] = useState<FrameId>(defaultFrameId as FrameId)
  const [selectedFrameColor, setSelectedFrameColor] = useState<string>(defaultColor)
  ...
  const currentColorConfig =
    currentFrameConfig.colors.find(color => color.value === selectedFrameColor) ??
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-black text-neutral-100">
          ...
          <div className="mx-auto flex max-w-[1400px] flex-col gap-10 px-4 lg:flex-row lg:items-start lg:px-10 xl:px-16">
            ...
            <div className="flex w-full max-w-md flex-col gap-6 lg:max-w-sm xl:max-w-md">
              <div>
                ...
                <h1 className="text-2xl font-semibold text-neutral-50 sm:text-3xl">{config.name}</h1>
                <p className="mt-3 text-sm leading-relaxed text-neutral-300">{config.description}</p>
              </div>
              ...
              <div className="rounded-3xl border border-white/10 bg-neutral-900/70 p-4 text-xs text-neutral-200 shadow-[0_24px_80px_rgba(0,0,0,0.6)]">
                <p className="mb-3 text-[0.65rem] font-semibold uppercase tracking-[0.18em] text-neutral-500">Highlights</p>
              ...
              <div>
                <p className="mb-2 text-[0.65rem] font-semibold uppercase tracking-[0.18em] text-neutral-500">Magnification</p>
              ...
              <div className="rounded-3xl border border-white/10 bg-neutral-900/80 p-4 text-xs text-neutral-200 shadow-[0_24px_80px_rgba(0,0,0,0.7)]">
                <p className="mb-3 text-[0.65rem] font-semibold uppercase tracking-[0.18em] text-neutral-500">Frame style</p>
                    ...
                    return (
                      ...
                      >
                        <div className="relative h-16 w-full">
                          <Image src={frame.baseImage} alt={frame.label} fill className="object-cover" />
                        </div>
                ...
                <div className="mb-2 text-[0.7rem] text-neutral-300">
                  ...
                  <p className="mt-1">
                    Choose a base frame, then fine-tune the finish. All frames support light mounts and prescription builds.
                  </p>
                </div>
                ...
                <div className="mt-4 w-full max-w-[400px] mx-auto">
                  <div className="relative w-full max-w-[400px] min-w-[200px] h-[180px] bg-neutral-800 rounded-xl overflow-hidden shadow-[0_4px_12px_rgba(0,0,0,0.3)] border-2 border-emerald-400/30">
                    <Image
              ...
              <div className="rounded-3xl border border-white/10 bg-neutral-900/90 p-4 shadow-[0_24px_90px_rgba(0,0,0,0.75)]">
        ...
        <section ref={techRef} className="border-t border-white/10 bg-black pb-24 pt-16">
          <div className="mx-auto flex max-w-[1200px] flex-col gap-10 px-4 lg:px-10 xl:px-16">
            <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-start">
              <div className="max-w-lg">
                <p className="mb-2 text-[0.65rem] font-semibold uppercase tracking-[0.25em] text-neutral-500">Technical detail</p>
                <h2 className="text-xl font-semibold text-neutral-50 sm:text-2xl">{config.specTitle}</h2>
                <p className="mt-3 text-sm leading-relaxed text-neutral-300">{config.specDescription}</p>
              </div>
        ...
        <section className="relative overflow-hidden border-t border-white/10">
          <div className="relative mx-auto flex max-w-[1400px] items-center justify-between px-4 py-16 lg:px-10 xl:px-16">
            <div className="relative z-10 max-w-xl space-y-3">
              ...
              <h3 className="text-xl font-semibold text-white sm:text-2xl">Designed in the OR, priced for reality.</h3>
              <p className="text-sm text-neutral-300">
                HeliosX exists because surgeons shouldn&apos;t have to choose between compromised tools and two months of rent. The work is demanding enough—the access should be too.
              </p>
              <button
                onClick={() => {
